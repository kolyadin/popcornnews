...
class PhotoArticleTagDataMap extends NewsTagDataMap {
	...
	function __construct(DataMapHelper $helper = null) {
		...
		$this->cleanStatement = $this->prepare("DELETE FROM pn_photoarticles_tags WHERE postId = :id");
		$this->insertStatement = $this->prepare("INSERT INTO pn_photoarticles_tags (postId, type, entityId) VALUES (:id, :type, :entityId)");
		$this->fidnByLinkStatement = $this->prepare("
            SELECT n.* FROM pn_photoarticles AS n
            INNER JOIN pn_photoarticles_tags AS t ON (n.id = t.postId)
            WHERE t.tagId = :id
        ");
	}
