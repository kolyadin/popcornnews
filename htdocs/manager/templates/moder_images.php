...
$q = 'SELECT COUNT(*) FROM popkorn_user_pix WHERE moderated=0 OR  moderated IS NULL';// count lines
...
?>
	...
	<td class="TFHeader">Разрешить</td>
	<td class="TFHeader">Удалить</td>
      </tr>
<?
$q = 'SELECT * FROM popkorn_user_pix ' .
     'WHERE moderated=0 OR  moderated IS NULL LIMIT ' . $_GET['begin'] . ', ' . $_GET['end'];
$result=mysql_query($q, $link);
if (is_resource($result))
{
  ...
  ?>
	...
	<td align="center"><input type="radio" name="im[<?=$row['id']?>]" value="1"></td>
	<td align="center"><input type="radio" name="im[<?=$row['id']?>]" value="2"></td>
      </tr>
  <?
  }
}
