...
$page_id=2;
$quiz_goodsid=26;
$questions_goodsid=31;
// статистика $answers_goodsid=32;
...
<table class="MainTable" cellspacing="0">
  <tr>
    ...
    <td class="head"><b>Опрос приостановлен</b></td>
    <td class="head">&nbsp;</td>
    <td class="head">&nbsp;</td>
  </tr>
  <tr>
    <td colspan="7" style="border-bottom:1px solid #555;padding-top:15px;font-weight:700;font-size:110%;">Действующие опросы</td>
  </tr>
<?
   $ddd=date("Ymd");
...
?>
  <tr>
    <td class="c1"><a href="challenge_twilight_words.php?pageid=2&id=<? print $s[0]; ?>"><? print $s["name"]; ?></a></td>
    ...
    <td class="c3 c4"><a href="challenge_twilight_words.php?pageid=700&id=<? print $s[0]; ?>"><nobr><img src="i/stat.gif" alt="Статистика" width="24" height="24" style="margin:-5px 6px -7px 0">Ответы</nobr></a></td>
    <td class="c3 c4"><a href="challenge_twilight_words.php?pageid=100&id=<? print $s[0]; ?>"><img src="i/delete.gif" alt="удалить конкурс" width="24" height="24" style="margin:-5px 6px -7px 0">Удалить</a></td>
  </tr>
<?      
   }
?>
  <tr>              
    <td colspan="7" style="border-bottom:1px solid #555;padding-top:15px;font-weight:700;font-size:110%;">Завершившиеся или приостановленные опросы</td>
  </tr>
<?
...
<form name="workform2" action="challenge_twilight_words.php" method="post" ENCTYPE="multipart/form-data">
...
<table cellpadding="4" cellspacing="1" border="0" bgcolor="#F9F9F8" class="FormTable">
  ...
  <tr>
    <td bgcolor="#D6DFF7" align="right"><b>Приостановить опрос</b></td>
...
<?
  ...
  case(2):
?>
<p><b><a href="challenge_twilight_words.php">к списку конкурсов</a></b></p>
<?  
...
<?/*<p style="margin:0px 0px 20px 0px;"><a href="challenge_twilight_words.php?pageid=700&id=<? print $s[0]; ?>"><nobr><img src="i/stat.gif" alt="Статистика" width="24" height="24" style="margin:0px 6px -7px 0">Посмотреть ответы</nobr></a></p>*/?>
...
if ($s["pole3"]) 
 {
  ...
  ?><a href="challenge_twilight_words.php?pageid=104&id=<? print $s[0]; ?>">удалить</a><?
 };
...
<?
   ...
   while($s=mysql_fetch_array($line))
   {
      $s['pole15'] = split(';', $s['pole15']);
?>

<textarea name="q<? print $s[0]; ?>name" class="cfi" rows=2 style="width:90%;height:60px;"><? print $s["name"]; ?></textarea><a href="challenge_twilight_words.php?pageid=103&qid=<? print $s[0]; ?>&id=<? print $id; ?>"><img src="i/i12.gif" alt="удалить вопрос" width="16" height="16" style="margin-top:-15px;"></a>
...
?>
  <tr>
    <td bgcolor="#E7E7E7" width="1"><input type="text" style="width: 90px;" name="q<? print $s[0]; ?>pole15[<?=($i-1);?>]" value="<?=$s['pole15'][$i-1];?>"></td>
...
<table cellpadding="4" cellspacing="1" border="0" bgcolor="#F9F9F8" class="MainTable">
  <tr>
    <td title="из папки Опрос для пользователей - Результат словами - Ключевые слова (если много то перечислите через запятую)">ID записи</td>
...
?>
  <tr>
    <td bgcolor="#E7E7E7" width="1"><input type="text" style="width: 90px;" name="qpole15[<?=$i?>]" value="<?=$s['pole15'][$i-1];?>"></td>
...
<h3><? print $item["name"]; ?></h3>
 ...
 <p>Опрос стартует: <b><? print dat_($item["pole4"], " "); ?></b></p>
 <p>Опрос заканчивается: <b><? print dat_($item["pole5"], " "); ?></b></p>

 <p style="margin:5px 0px;">В опросе приняли участие человек: <b><? print ($s1[0]+$s2[0]); ?></b></p>
...
<table cellspacing="0" class="MainTable">
  <tr>
    ...
    <td class="head" width="20%">Телефон</td>
    <td class="head" width="20%">Дата ответа</td>
    <td class="head" width="20%">Ответ</td>
...
?>
<tr>
 <td class="c1" width="20%"><? print $s["name"]; ?>&nbsp;</td>
 <td class="c1" width="20%"><? print $s["pole5"]; ?>&nbsp;</td>
 <td class="c1" width="20%"><? print $s["pole6"]; ?>&nbsp;</td>
 <td class="c2" width="20%"><? print dat_($s["pole8"], " "); ?></td>
 <td class="c3" width="20%"><? print eregi_replace("-","&nbsp;",$s["pole3"]); ?></td>
 <td class="c3" width="20%"><? print $s["pole7"]; ?></td>
</tr>
<?
      }
       else
...
?>
<tr>
 <td class="c1" width="20%"><a href="/profile/<? print $s["pole1"]; ?>" target="_blank"><? print $s["name"]; ?></a></td>
 <td class="c1" width="20%"><? print $s["pole5"]; ?></td>
 <td class="c1" width="20%"><? print $s["pole6"]; ?>&nbsp;</td>
 <td class="c2" width="20%"><? print dat_($s["pole8"], " "); ?></td>
  <td class="c3" width="20%"><? print eregi_replace("-","&nbsp;",$s["pole3"]); ?></td>
...
<?
  ...
  case(101):
    ...
    header ("location:challenge_twilight_words.php");
    exit(1);
  break;
  ...
  case(102): 
    ...
    {
      $qid =         $s[0];
      $qname =       $_POST['q'.$s[0].'name'];
      $qpole2 =      $_POST['q'.$s[0].'pole2'];
      $qpole3 =      $_POST['q'.$s[0].'pole3'];
      $qpole4 =      $_POST['q'.$s[0].'pole4'];
      $qpole5 =      $_POST['q'.$s[0].'pole5'];
      $qpole6 =      $_POST['q'.$s[0].'pole6'];
      $qpole7 =      $_POST['q'.$s[0].'pole7'];
      $qpole8 =      $_POST['q'.$s[0].'pole8'];
      $qpole9 =      $_POST['q'.$s[0].'pole9'];
      $qpole10 =     $_POST['q'.$s[0].'pole10'];
      $qpole11 =     $_POST['q'.$s[0].'pole11'];
      $qpole15 =     join(';', $HTTP_POST_VARS['q'.$s[0].'pole15']);
    ...
    header ("location:challenge_twilight_words.php?id=$id&pageid=2");
    exit(1);
  break;
