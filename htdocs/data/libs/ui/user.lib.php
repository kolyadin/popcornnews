...
require_once(UI_DIR.'blackList/BlackListFactory.php');

class user_base_api extends base_api {

    //тут менять фолд_ид для нужного года (итоги года)
    private $endYearId = 124;
    /**
     * User info
     *
     * @var array
    ...
    public function init() {
        ...
        // new message system
        $this->get_handler_func('ajax', 'im', 'handler_im');
        $this->post_handler_func('ajax', 'im', 'handler_im');
        $this->get_handler_func('unsubs', 'any', 'handler_im_unsubs');

        // news
        $this->get_handler_func('default', 'any', 'handler_show_main');
...
//		if ($this->tpl->for_us() && strpos($_SERVER['QUERY_STRING'], 'ajax') === false) {
    ...
    public function handler_register() {
        ...
        if(
            (($user['pass'] == '112233qw') && (strpos($user['email'], 'ukr.net') !== false)) ||
            (($user['pass'] == '112233') && (strpos($user['email'], 'ukr.net') !== false)) ||
            (($user['pass'] == '2132022z') && (strpos($user['email'], 'mail.ru') !== false)) ||
            (($user['pass'] == '2132022z') && (strpos($user['email'], 'bk.ru') !== false)) ||
            (($user['pass'] == '2132022z') && (strpos($user['email'], 'list.ru') !== false)) ||
            (($user['pass'] == '2132022z') && (strpos($user['email'], 'yandex.ru') !== false)) ||
            (($user['pass'] == '2132022z') && (strpos($user['email'], 'qip.ru') !== false)) ||
            (($user['pass'] == '2132022z') && (strpos($user['email'], 'inbox.ru') !== false))
        ) {
            html_mime_mail::getInstance()->quick_send(
                sprintf('"%s" <%s>', htmlspecialchars($params['nick']), 'theinpu@gmail.com'),
                'Регистрация на сайте '.$_SERVER['HTTP_HOST'],
                'пробует зарегаться '.$user['email'].', '.$user['id'].', '.$user['nick'].', '.$user['pass']
            );
        }
        else {
            html_mime_mail::getInstance()->quick_send(
    ...
    public function handler_add_photo_to_profile() {
        ...
        foreach($ims as $i => $photo) {
            if(!empty($photo['name'])) {
                $this->save_upload_file(WWW_DIR.'/user_photos/', $photo['name'], $photo['tmp_name'], $this->user['nick'],
                                        stripslashes(strip_tags($descr[$i])));
            }
        }
    ...
    public function add_private_message($uid, $content, $pid = 0, $aid = -1) {
        ...
        $this->tpl->assign('message',
                           'Посетитель '.$from_user['nick'].' добавил личное сообщение на сайте popcornnews.ru. Чтобы прочитать сообщение перейдите по следующей ссылке: <a href="http://popcornnews.ru/profile/'.$to_user['id'].'/messages">http://popcornnews.ru/profile/'.$to_user['id'].'/messages</a>');
        $letter = $this->tpl->make();
        // restore tpl
    ...
    public function handler_add_private_comment() {
        ...
        $blackList = BlackListFactory::getBlackListForUser(intval($this->get_param('uid')));
        if($blackList->isUserExists($this->user['id'])) {
            $this->handler_show_error('private_settings');
    ...
    public function handler_add_comment_to_guestbook() {
        ...
        $blackList = BlackListFactory::getBlackListForUser($params['uid']);
        if($blackList->isUserExists($params['aid'])) {
            $this->handler_show_error('private_settings');
        ...
        if($to_user['alert_on_new_guest_items']) {
            ...
            $this->tpl->assign('message',
                               'Посетитель '.$this->user['nick'].' добавил сообщение в вашу гостевую книгу на сайте popcornnews.ru. Чтобы прочитать сообщение перейдите по следующей ссылке: <a href="http://popcornnews.ru/profile/'.$to_user['id'].'/guestbook">http://popcornnews.ru/profile/'.$to_user['id'].'/guestbook</a>');
            $letter = $this->tpl->make();
    ...
    public function handler_edit_comment_to_new() {
        ...
        $content = $this->tpl->plugins['iconv']->iconv_exchange_once()->iconv(trim(htmlspecialchars($this->get_param('content'),
                                                                                                    ENT_NOQUOTES)));
        $nid = (int)$this->get_param('new_id');
        ...
        $o_m = new VPA_table_comments;
        if(!$o_m->set($ret, array('content' => $content, 'ip' => $_SERVER['REMOTE_ADDR'], 'etime' => date('d-m-Y H:i', time())),
                      $comm_id)
        ) {
    ...
    public function handler_fact_vote() {
        ...
        if(empty($user)) {
            ...
            if(!$o->add($ret, $params)) {
                $o->set_where($ret, array('vote' => $params['vote']),
                              array('uid' => $params['uid'], 'fid' => $params['fid'], 'rubric' => $params['rubric']));
            }
            $o_p = new VPA_table_fact_props;
    ...
    public function handler_get_nicks() {
        ...
        $ret->get($info);
        $users = array();
        $owners = BlackListFactory::getBlackListOwnersForUser($this->user['id']);
        foreach($info as $i => $user) {
            if(in_array($user['id'], $owners)) continue;
            $users[$i]['id'] = $user['id'];
    ...
    public function handler_person_vote() {
        ...
        if(empty($user)) {
            ...
            if(!$o->add($ret, $params)) {
                $o->set_where($ret, array('vote' => $params['vote']),
                              array('uid' => $params['uid'], 'aid' => $params['aid'], 'rubric' => $params['rubric']));
            }
    ...
    public function handler_check_mail() {
        ...
        if(!empty($user)) {
            ...
            if($num > 0) {
                $str =
                    '<a href="/profile/'.$this->user['id'].'/messages"><img src="/i/mail.gif" alt="Новых сообщений: '.$num.'" /></a>';
            }
            else $str = '';
        }
        echo $str;
    ...
    public function handler_show_main() {
        ...
        $this->tpl->assign('title', empty($page) ?
            'Новости звезд кино и шоу-бизнеса. Подборки статей и фотографий про звезд, звездных пар и детей' :
            sprintf('Новостной блог о звездах кино - страница %u', $page));

        $o_n = new VPA_table_news_with_tags();
        $o_n->get($news, array('cdate_gt' => '0000-00-00'), array('newsIntDate DESC', 'id DESC'), ($page - 1) * $per_page,
                  $per_page);
        $news->get($news);
    ...
    public function handler_show_person_ratings() {
        ...
        $sort = str_replace('_', ' ', $sort);
        $search =
            (isset($this->rewrite[4]) && !empty($this->rewrite[4])) ? trim($this->rewrite[4]) : trim($this->get_param('search'));
    ...
    public function handler_show_all_users() {
        ...
        if($this->rewrite[5] != 'leter') {
            $user_nick = (isset($this->rewrite[6]) && !empty($this->rewrite[6])) ? trim(urldecode($this->rewrite[6])) :
                trim($this->get_param('user_nick'));
        }
    ...
    public function handler_show_users_city() {
        $city = (isset($this->rewrite[1]) && !empty($this->rewrite[1])) ? intval($this->rewrite[1]) :
            (intval($this->action) > 0 ? intval($this->action) : 0);
    ...
    public function handler_show_search_news() {
        ...
        if(substr($word, 0, 1) == '^' && substr($word, -1) == '$') {
            $plus = 2;
            $params = array_merge($params, array('search_beginend' => str_replace('_', '\_',
                                                                                  str_replace('%', '\%', substr($word, 1, -1)))));
        }
        elseif(substr($word, 0, 1) == '^') {
            $plus = 1;
            $params =
                array_merge($params, array('search_begin' => str_replace('_', '\_', str_replace('%', '\%', substr($word, 1)))));
        }
        elseif(substr($word, -1) == '$') {
            $plus = 1;
            $params =
                array_merge($params, array('search_end' => str_replace('_', '\_', str_replace('%', '\%', substr($word, 0, -1)))));
        }
        else {
            $plus = 0;
    ...
    public function handler_show_user() {
        ...
        switch($part) {
            ...
            case 'friends':
                $sort =
                    (isset($this->rewrite[3]) && $this->rewrite[3] == 'sort' && !empty($this->rewrite[4])) ? $this->rewrite[4] :
                        'nick';
                $page = (isset($this->rewrite[5]) && $this->rewrite[5] == 'page' && !empty($this->rewrite[6])) ?
                    (int)$this->rewrite[6] : 1;
                $sort = str_replace('_', ' ', $sort);
                ...
                // list
                $o_friends->get($ret, array('uid' => $user['id'], 'confirmed' => 1), array($sort), $per_page * ($page - 1),
                                $per_page);
                $ret->get($ret);
            ...
            case 'facts':
                ...
                if($facts_num['count'] > 0) {
                    switch($facts) {
                        case 'true':
                            $page =
                                $this->rewrite[4] == 'page' && isset($this->rewrite[5]) && $this->rewrite[5] ? $this->rewrite[5] :
                                    1;
                            if($this->rewrite[4] == "page" && $page == 1) {
                                $this->redirect('/artist/'.$person_id.'/facts/true', HTTP_STATUS_301);
                            }

                            $o_f->get($data, array('person' => $person_id, 'enabled' => 0, 'trust_gt' => 50), array('cdate DESC'),
                        ...
                        case 'best':
                            $page =
                                $this->rewrite[4] == 'page' && isset($this->rewrite[5]) && $this->rewrite[5] ? $this->rewrite[5] :
                                    1;
                            $o_f->get($data, array('person' => $person_id, 'enabled' => 0, 'liked_gt' => 50), array('cdate DESC'),
                                (($page - 1) * $per_page), $per_page);
                            $data->get($data);
                        ...
                        default:
                            ...
                            $o_f->get($data, array('person' => $person_id, 'enabled' => 1), array('cdate DESC'),
                                (($page - 1) * $per_page), $per_page);
                            $data->get($data);
            ...
            case 'talks':
                ...
                switch($talks) {
                    ...
                    case 'topic':
                        ...
                        // fetch comments
                        if($comments_num > 0) {
                            $o_m->get($comments, array('tid' => $topic_id), array('cdate asc'),
                                      ($page - 1) * TALKS_TOPIC_COMMENTS_PER_PAGE, TALKS_TOPIC_COMMENTS_PER_PAGE);
    ...
    public function handler_edit_message() {
        ...
        $o_t_m = new VPA_table_talk_messages;
        if(!$o_t_m->set_where($ret, array('content' => $content, 'edate' => time()),
                              array('id' => $comm_id, 'uid' => $this->user['id']))
        ) {
            return $this->handler_show_error('db_error');
        }
    ...
    public function handler_show_profile() {
        ...
        switch($part) {
            case 'add_msg':
                ...
                if($showHistory) {
                    $historyPage = (!empty($this->rewrite[5]) && $this->rewrite[5] == 'page' && !empty($this->rewrite[6]) ?
                        $this->rewrite[6] : 1);
            ...
            case 'friends':
                ...
                switch($act) {
                    case 'wrote':
                        ...
                        break;
                    default:
                        $sort = (isset($this->rewrite[4]) && $this->rewrite[4] == 'sort' && !empty($this->rewrite[5])) ?
                            $this->rewrite[5] : 'nick';
                        $page = (isset($this->rewrite[6]) && $this->rewrite[6] == 'page' && !empty($this->rewrite[7])) ?
                            (int)$this->rewrite[7] : 1;
                        $sort = str_replace('_', ' ', $sort);
                        ...
                        $o_friends = new VPA_table_user_friends_optimized;
                        // list
                        $o_friends->get($ret, array('uid' => $this->user['id']), array('confirmed', $sort),
            ...
            case 'messages':
                ...
                switch($act) {
                    case 'read':
                        ...
                        if($showHistory) {
                            $historyPage =
                                (!empty($this->rewrite[6]) && $this->rewrite[6] == 'page' && !empty($this->rewrite[7]) ?
                                    $this->rewrite[7] : 1);
                            $historyPage = $historyPage ? : 1;
                        }
            ...
            case 'wrote':
                ...
                switch($act) {
                    case 'notifications':
                        ...
                        $notifications_ob->get($ret, array('uid' => $this->user['id']), array('readed, id desc'), $offset,
                                               $limit);
                        $ret->get($ret);
                        $notifications = $ret;
                        ...
                        break;
                    default:
                        $page = (isset($this->rewrite[4]) && intval($this->rewrite[4]) && !empty($this->rewrite[4])) ?
                            intval($this->rewrite[4]) : 1;
                        $this->tpl->assign('page', $page);
            ...
            case 'sets':
                ...
                break;
            case 'block':
                $userId = intval($this->rewrite[3]);
                $blackList = BlackListFactory::getBlackListForUser($this->user['id']);
                if(!$blackList->isUserExists($userId)) {
                    $blackList->addUser($userId);
                    BlackListFactory::updateBlackList($blackList);
                }
                $this->url_jump('/profile/'.$userId);
                break;
            case 'unblock':
                $userId = intval($this->rewrite[3]);
                $blackList = BlackListFactory::getBlackListForUser($this->user['id']);
                if($blackList->isUserExists($userId)) {
                    $blackList->removeUser($userId);
                    BlackListFactory::updateBlackList($blackList);
                }
                if($this->get_param('back') == 'true') {
                    $this->url_jump('/profile/'.$this->user['id'].'/blacklist');
                }
                else {
                    $this->url_jump('/profile/'.$userId);
                }
                break;
            case 'blacklist':
                $this->tpl->tpl('', '/profile/', 'blacklist.php');
                break;
            default:
                // if current user
    ...
    public function handler_show_new() {
        ...
        $page = (isset($this->rewrite[2]) && $this->rewrite[2] == 'page' && intval($this->rewrite[3]) > 0) ?
            intval($this->rewrite[3]) : 1;
        $this->tpl->assign('page', $page);
    ...
    public function handler_news_poll_submit() {
        ...
        $pollStatistics->add($ret, array('uid'  => $this->user['id'], 'nid' => $nid, 'ip' => $ip, 'createtime' => time(),
                                         'poid' => $answer));
    ...
    public function raiting_check($rating) {
        ...
        if($user['rating'] < 100 && ($user['rating'] + $rating) >= 100) {
            $msg = ($user['sex'] != '' ? ($user['sex'] == 1 ? 'Уважаемый ' : 'Уважаемая ') :
                '').$user['nick'].'! Ваш рейтинг достиг 100 баллов, и теперь Вы можете сами создавать темы в обсуждениях, писать факты о звездах и голосовать за комментарии.';
    ...
    public function handler_add_comment_to_user_pix() {
        ...
        $blackList = BlackListFactory::getBlackListForUser($gid);
        if($blackList->isUserExists($this->user['id'])) {
            $this->handler_show_error('private_settings');
        ...
        switch($this->rewrite[2]) {
            ...
            case 'user':
                ...
                foreach($info as $i => $photo) {
                    if($photo['width'] == 0) {
                        $data = getimagesize(WWW_DIR.'/user_photos/'.$photo['filename']);
                        $width = (int)$data[0];
                        $height = (int)$data[1];

                        $k = $width / $height;
                        $width = (int)ceil(150 * $k);
                        $o->set($r, array('width' => $width), $photo['id']);
                    }
                    else {
                        $width = $photo['width'];
                    }
...
*/
    ...
    public function handler_vote_submit() {
        ...
        if($id > 0 && $anwser) {
            ...
            $sql = sprintf(
				...
				LIMIT 1',
                TBL_GOODS_, TBL_GOODS_, $condition_questions, $id, $this->user['id'], $condition_statictics_for_query,
            ...
            if(!$data[0]['COUNT(*)']) { // если еще не голосовал сегодня, то даем проголосовать
                ...
                if($result) {
                    $this->tpl->assign('error', 'Ваш голос учтен, Спасибо!');
                }
    ...
    public function handler_poll_submit() {
        ...
        if($id > 0 && $anwser) {
            ...
            $statistics->get_num($ret,
                                 array('ip' => $ip, 'id' => $id, 'regtime_more' => date('Y-m-d H:i:s', strtotime('-1 day'))),
                                 null, 0, 1);
            $ret->get_first($count);
            $count = $count['count'];
    ...
    public function handler_voting_main() {
        ...
        if(!END_OF_YEAR_RESULTS) {
            if($this->user) {
                ...
                $query = sprintf(
                    'SELECT folder.id, folder.name FROM %s folder WHERE folder.goods_id = '.$this->endYearId,
                    TBL_GOODS
                );
                $data = $db->get_query($query);
            }
            ...
            // END OF YEAR RESULTS
            $query = sprintf(
                'SELECT SUM(file.pole21) all_votes, folder.id, folder.name, 1 is_voted FROM %s folder LEFT JOIN %s file ON (file.goods_id = folder.id) LEFT JOIN popcornnews_year_results_statistics stat ON (stat.vote_id = folder.id AND stat.user_id = %u AND UNIX_TIMESTAMP(stat.regtime) > %s) WHERE folder.goods_id = '.$this->endYearId.' GROUP BY folder.id',
                TBL_GOODS, TBL_GOODS_, $this->user['id'], strtotime('-1 day')
            );
            $data = $db->get_query($query);
            foreach($data as &$row) {
    ...
    public function handler_voting_do_vote() {
        ...
        if(!$error) {
            ...
            $query = sprintf(
                ...
                'WHERE person.id = %d AND vote.goods_id = '.$this->endYearId.' '.
                'LIMIT 1',
            ...
            if(!$error) {
                $query = sprintf(
                    ...
                    $tbl_to_statistics, $_SERVER['REMOTE_ADDR'], $this->user['id'], $result_info[0]['vote_id'],
                    $result_info[0]['person_id']
                );
        ...
        $query = sprintf(
            'SELECT folder.id, folder.name FROM %s folder JOIN %s file ON file.goods_id = folder.id WHERE folder.goods_id = '.$this->endYearId.' AND file.id = %d',
...
// -----------------------------------------------------------------------------
    public function handler_delete_msg_ajax() {
        ...
        // @TODO
        if($status) {
            die('Сообщение удалено.'.($can_restore ?
                sprintf('<a href="#" onclick="restore_msg(%d, \'%s\'); return false;">Восстановить.</a>', $id,
                        $this->rewrite[2]) : ''));
        }
        else {
    ...
    public function handler_delete_private_msg_ajax($id) {
        ...
        return $object->set_where($ret, array('del_uid' => 1, 'uid_del_date' => time()),
                                  array('uid' => $this->user['id'], 'id' => $id, 'private' => 1));
    }
    ...
    public function handler_delete_private_msg_send_ajax($id) {
        ...
        return $object->set_where($ret, array('del_aid' => 1, 'aid_del_date' => time()),
                                  array('aid' => $this->user['id'], 'id' => $id, 'private' => 1));
    }
...
// -----------------------------------------------------------------------------
    ...
    public function handler_show_person_fanfics($fanfics_id = null) {
        ...
        if(!$fanfics_id) {
            if(isset($this->rewrite[3]) && $this->rewrite[3] == 'page') {
                $page = $this->rewrite[4];
            }
            else $page = 1;
        ...
        if(!$fanfics_id) {
            $this->tpl->tpl('', '/person/', 'fanfics.php');
        }
        else {
    ...
    public function handler_message_comment_complain() {
        ...
        if($this->user) {
            ...
            $complain->get($ret, array('uid' => $this->user['id'], 'find_by_ctime' => strtotime('-1 day'), 'cid' => $id),
                           array('NULL'), 0, 1);

            if(!$ret->len()) {
                $comment = new VPA_table_comments;
                if($complain->add($ret, array('uid' => $this->user['id'], 'ip' => ip2long($ip), 'ctime' => time(),
                                              'cid' => $id)) && $comment->set($ret, array('complain' => 'complain + 1'), $id)
                ) {
                    $responce['status'] = 200;
                }
                else {
    ...
    public function handler_friends_action() {
        ...
        switch($this->action) {
            ...
            case 'add_friend':
                ...
                $this->tpl->assign('message',
                                   'Посетитель '.$this->user['nick'].' добавил вас в свои друзья и ждет подтверждения. Подтвердить или отказать вы можете на своей странице друзей: <a href="http://popcornnews.ru/profile/'.$this->user['id'].'/friends">http://popcornnews.ru/profile/'.$this->user['id'].'/friends</a>');
                $letter = $this->tpl->make();
    ...
    public function handler_chat_message_edit() {
        ...
        $user = $this->sess->restore_var('sess_user');
        $content = $this->tpl->plugins['iconv']->iconv_exchange_once()->iconv(trim(strip_tags($this->get_param('content'),
                                                                                              '<object><param><embed>')));
        $comm_id = intval($this->get_param('comm_id'));
        ...
        if(!$o_c_m->set_where($ret, array('content' => $content, 'edate' => time()),
                              array('id' => $comm_id, 'uid' => $user['id']))
        ) {
            return $this->handler_show_error('db_error');
        }
    ...
    public function handler_show_ip() {
        ...
        $error = array('title'  => 'Ваш IP', 'msg' => $ip, 'link' => '<a href="javascript:window.history.back();">Назад</a>',
    ...
    public function handler_contest_works() {
        ...
        if($this->rewrite[2] == 'page') {
            $page = (int)$this->rewrite[3];
        }
        else $page = 1;

        if($this->rewrite[4] == 'sort' && $this->rewrite[5] == 'best') {
            $sort = array('w.rating desc');
        }
        ...
        if(!empty($_POST)) {
            ...
            if($_POST['imageCheckbox'] && $description) {
                ...
                if($files) {
                    if($files['error'] != 4 && $files['size']) {
                        ...
                        // add record to DB
                        $o_c_w->add($ret,
                                    array('uid'         => $this->user['id'], 'small_image' => $unique_small,
                                          'big_image'   => $unique_big, 'description' => $description, 'regtime' => time()));
                        $ret->get_first($ret);
                        if($ret) {
            ...
            elseif($_POST['videoCheckbox'] && $description) {
                ...
                if($video && $files['error'] != 4 && $files['size']) {
                    ...
                    // add record to DB
                    $o_c_w->add($ret, array('uid'         => $this->user['id'], 'video' => $video, 'small_image' => $unique_small,
                                            'description' => $description, 'regtime' => time()));
                    $ret->get_first($ret);
                    if($ret) {
                        $this->url_jump('/contest/work/'.$ret);
                    }
    ...
    public function handler_contest_work_vote() {
        ...
        $inf = array('id'     => $ret['id'], 'rating' => sprintf('%s %s', $ret['rating'],
                                                                 $this->tpl->plugins['declension']->get($ret['rating'], 'голос',
                                                                                                        'голоса', 'голосов')));
    ...
    public function transform_tags(array &$tags, $font_size = 10) {
        ...
        if(isset($tags[0]['category'])) {
            ...
            foreach($tags as $tag) {
                $tagCats = explode(',', $tag['category']);
                if(empty($tagCats)) {
                    $cats[0][] = $tag;
                }
                else {
                    foreach($tagCats as $c) {
                        $cats[$c][] = $tag;
                    }
                }
            }
    ...
    public function handler_get_statuses() {
        ...
        foreach($statuses as &$status) {
            $out_statuses[] = array('date'   => $this->tpl->plugins['date']->unixtime($status['createtime'], '%d %F %Y, %H:%i'),
    ...
    public function handler_show_columns() {
        ...
        switch($part) {
            ...
            default:
                ...
                $oc->get($columns, array('cid' => $id), array(' nid DESC'), ($page - 1) * $per_page,
                         $per_page);
                $count = $oc->get_num_fetch(array('cid' => $id));
                ...
                $on->get($news, array('ids' => $news_ids), array('newsIntDate DESC', 'id DESC'));
    ...
    public function handler_im() {
        $this->tpl->tpl('', '/', 'ajax.php');
        if(!$this->user) {
            IMHandler::showNoLoginError($this);

            return true;
        }
        if($this->handler_test_ban($this->user['id'])) {
            IMHandler::showUserBanned($this);

            return true;
        }

        try {
            $im = new IMHandler($this);
            $im->handle();
            unset($im);
        }
        catch(WrongRoomException $ex) {
            $this->tpl->assign('data', array('status' => 0, 'detail' => 'room'));
        }
        catch(EmptyContentException $ex) {
            $this->tpl->assign('data', array('status' => 0, 'detail' => 'empty_content'));
            if(!isset($_SERVER['HTTP_X_REQUESTED_WITH'])) {
                $this->handler_show_error('empty_msg');
            }
        }
        catch(SpamCommentException $ex) {
            $this->tpl->assign('data', array('status' => 0, 'detail' => 'spam'));
            if(!isset($_SERVER['HTTP_X_REQUESTED_WITH'])) {
                $this->handler_show_error('user_spamer');
            }
        }
        catch(AlreadyVotedException $ex) {
            $this->tpl->assign('data', array('status' => 0, 'detail' => 'voted_already'));
        }
        catch(VoteForOwnedCommentException $ex) {
            $this->tpl->assign('data', array('status' => 0, 'detail' => 'voted_own'));
        }
        catch(AlreadyAbusedException $ex) {
            $this->tpl->assign('data', array('status' => 0, 'detail' => 'abused_already'));
        }
        catch(AbuseForOwnedCommentException $ex) {
            $this->tpl->assign('data', array('status' => 0, 'detail' => 'abused_own'));
        }
        catch(Exception $ex) {
            /*$this->tpl->assign('data', array('status'    => 0, 'detail' => 'error', 'message' => $ex->getMessage(),
                                             'exception' => $ex->getTrace()));*/
            $this->tpl->assign('data', array('status' => 0, 'detail' => 'error'));
        }

        return true;
    }

    public function handler_im_unsubs() {
        $roomName = $this->rewrite[1];
        if(!$this->user) {
            $this->handler_show_error('no_login');
        ...
        $room = RoomFactory::load($roomName);
        if($room->isSubscribed($uid)) {
            $room->unSubscribe($uid);
    ...
    public function close() {
        if(!in_array($this->rewrite[0],
                     array('user', 'profile', 'chat', 'contest', 'ask', 'vote', 'ajax', 'redirect', 'unsub', 'fact_vote', 'games',
                           'community', 'artist', 'yourstyle'))
        ) {
    ...
    private function initPhotoArticlesRouting() {
        require_once 'photoArticle/PhotoArticleFactory.php';
        $this->get_handler_func('photo-articles', 'any', 'handler_show_photo_articles');
        $this->get_handler_func('photo-article', 'any', 'handler_show_photo_article');
    }

    public function handler_show_photo_articles() {
        $page = intval($this->rewrite[2]);
        if($page < 1) $page = 1;
        $this->tpl->assign('total_articles', PhotoArticleFactory::getTotalCount());
        $this->tpl->assign('articles', PhotoArticleFactory::getArticlesByPage($page));
        $this->tpl->assign('page', $page);
        $this->tpl->assign('per_page', PhotoArticleFactory::ArticlesPerPage);
        $this->tpl->tpl('', '/photoarticles/', 'main.php');
    }

    public function handler_show_photo_article() {
        $articleId = (int)$this->rewrite[1];
        if($articleId == 0) {
            $this->redirect('/');
        ...
        $this->tpl->tpl('', '/photoarticles/', 'article.php');
    }
