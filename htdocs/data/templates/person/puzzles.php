<?
$this->_render('inc_header', array('title'=>'Пазлы  - ' . $d['person']['name'], 
...
<div id="contentWrapper" class="twoCols">
	...
	<div id="content">
		<ul class="menu">
			...
			<li class="active"><a href="/persons/<?=$handler->Name2URL($d['person']['eng_name']);?>/puzli">пазлы</a></li>
			<?}?>
		...
		<h2>Пазлы <?=$d['person']['genitive']?></h2>
		<ul class="imagesList puzzlesList">
			<?
			$puzzles = $p['query']->get('puzzles', array('person'=>$d['person']['id']), array('id'), null, null);
			foreach ($puzzles as $i => $puzzle) {?>
			<li><a class="img" href="#" onClick="open_puz('<?=str_replace("'", "\'", $this->getStaticPath('/upload/' . $puzzle['big_image']))?>'); return false;"><img src="<?=$this->getStaticPath('/upload/_75_75_90_' . $puzzle['big_image'])?>" alt="" /></a></li>
