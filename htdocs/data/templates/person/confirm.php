...
$this->_render('inc_header', array(
	'title' => $d['person']['name'] . ' новости, ' . $d['person']['eng_name'] . ' фото, биография, обои ' . $d['person']['genitive'],
...
<div id="contentWrapper" class="twoCols">
	<div id="content">
		...
		<div class="systemMessage">
			<?if ($d['act'] == 1) {?>
			<h4>Вы хотите стать поклонником и вступить в группу <?=$d['person']['name']?>?</h4>
			<p>После этого вы появитесь в списке поклонников, и так-же сможете читать новости о звезде через свой профайл.</p>
			<?} elseif ($d['act'] == 2) {?>
			<h4>Вы хотите покинуть группу <?=$d['person']['name']?>?</h4>
			<?}?>
		</div>
		<form class="add" method="post" name="fr" action="/">
			<input type="hidden" name="type" value="person" />
			<input type="hidden" name="action" value="fans" />
			<input type="hidden" name="person" value="<?=$d['person']['id']?>" />
			<input type="hidden" name="act" value="<?=$d['act']?>" />
