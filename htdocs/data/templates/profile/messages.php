...
<div id="contentWrapper" class="twoCols">
	<div id="content">
		...
		<ul class="menu bLevel">
			<li class="active"><a rel="nofollow" href="/profile/<?=$d['cuser']['id']?>/messages">принятые</a></li>
		...
		<div class="trackContainer commentsTrack">
			...
			$num_records = $p['query']->get_num('user_msgs', array('uid' => $d['cuser']['id'], 'private' => 1, 'del_uid' => 0));
			$pages = ceil($num_records / $limit);
			foreach ($p['query']->get('user_msgs', array('uid' => $d['cuser']['id'], 'private' => 1, 'del_uid' => 0), array('readed asc', 'id desc'), $offset, $limit) as $i => $msg) {
			...
			<div class="trackItem<?=($msg['readed'] == 0 ? ' notReaded' : null)?>" id="<?=$msg['id'];?>">
				<div class="post">
					...
					<div class="details">
						...
						<?if ($msg['aid'] != 57) { // id пользователя администрации?>
						<a href="/profile/<?=$d['cuser']['id']?>/messages/answer/<?=$msg['id']?>">ответить</a>
						<?}?>
						<a href="#" onclick="delete_msg(<?=$msg['id'];?>, 'private'); return false;">удалить</a>
		...
		<div class="noUpperBorder paginator">
			...
			<ul>
				...
				<li>
					<?if (!isset($pi['current'])) {?>
					<a rel="nofollow" href="/profile/<?=$d['cuser']['id']?>/messages/page/<?=$pi['link']?>"><?=$pi['text']?></a>
					<?} else {?>
					<?=$pi['text']?>
					<?}?>
				</li>
