...
<html>
	<head>
		...
		<meta http-equiv="Cache-Control" content="no-cache"><link rel="stylesheet" type="text/css" href="/manager/styles/global.css">
	...
	<body>
		<a name="topper"></a><table cellspacing="1" class="TableFiles">
			...
			<?
			foreach ($p['query']->get('countries', null, array('rating'), null, null) as $i => $country) {?>
			<tr>
				<td><?=$country['id']?></td>
				<td width="70%"><a href="admin.php?type=cities&country_id=<?=$country['id']?>"><?=$country['name']?></a></td>
				<td><?=$country['rating']?></td>
				<td><a href="admin.php?type=country&action=edit&id=<?=$country['id']?>#form">&raquo;</a></td>
				<td align="center"><a href="admin.php?type=country&action=del&id=<?=$country['id']?>" onclick="return window.confirm('Вы точно хотите удалить эту страну ?');">&raquo;</a></td>
			</tr>
				<?
			}
			?>
			<tr><td class="TFHeader" colspan="7" align="right"><a href="admin.php?type=countries#form">Добавить новую</a></td></tr>
		</table>
		<a name="form"></a>
		<form method="POST" action="admin.php" name="frm" class="Fform">
			...
			<input type="hidden" name="id" value="<?=$d['edit_id']?>">
				<?
				$edit = array_shift($p['query']->get('countries', array('id'=>$d['edit_id']), null, 0, 1));
			}?>
			<div class="FName">
