...
<div>
	<div class="paginator smaller">
		...
		<ul>
			...
			<li>
				<?if (!isset($pi['current'])) {?>
				<a href="?type=yourstyle&action=brandTiles&bid=<?=$d['bid']?>&page=<?=$pi['link']?>"><?=$pi['text']?></a>
				<?} else {?>
				<?=$pi['text']?>
				<?}?>
			</li>
...
<table cellspacing="1" class="TableFiles">
	...
	<tr>
		<td><a href="?type=yourstyle&action=groupsTiles&gid=<?=$tile['gid']?>"><?=$tile['group'] ?: 'Не задана'?></a></td>
		...
		<td>
			...
			<a href="?type=yourstyle&action=duplicateGroupsTile&tid=<?=$tile['id']?>">Дублировать</a>
		</td>
	</tr>
	<?}?>
</table>
