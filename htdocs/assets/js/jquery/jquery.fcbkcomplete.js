...
jQuery(function($) {
    $.fn.fcbkcomplete = function(opt) {
        return this.each(function() {
            ...
            function preSet() {
                var option;
                for (var i in options.preset_elements) {
                    option = options.preset_elements[i];
                    addItem(option.name, option.id, 0, 0);
                    cache.push({
                        key: option.id,
                        value: option.name,
                    ...
                    search_string += "" + (cache.length - 1) + ":" + option.name + ";";
                }
            }

            //public method to add new item
            $(this).bind("addItem", function(event, data) {
                addItem(data.value, data.key, 0, 0, 0);
            });
		    ...
		    try {
			$(holder).children('li.bit-box').each(function(i, item) {
                        ...
                        if (options.json_url) {
                            if (options.cache && json_cache) {
                                ...
                                setTimeout(function() {
                                    ...
                                    $.getJSON(options.json_url, {q: etext, limit: options.maxitems}, function(data) {
                ...
                while (match != null && maximum > 0) {
                    ...
                    if (options.filter_selected && element.children("option[value=" + object.value + "]").hasClass("selected")) {
                        ...
                        content += '<li rel="' + object.key + '">' +
                                   ...
                                   '<div class="item">' + itemIllumination(object.value, etext) + '</div>' +
            ...
            function bindEvents() {
                ...
                feed.children("li").mousedown(function() {
                    var option = $(this);
                    addItem(option.children('div.item').text(), option.attr("rel"), 0, 0, 1);
                    feed.hide();
                    complete.hide();
                });
		    ...
		    var to = 0;
		    try {
			var to = item.get(0).attributes.length;
		    } catch (e) {}
                for (i = 0; i < to; i++) {
